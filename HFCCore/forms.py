...
class Problem_Statement_form(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
          ...
          'name':Textarea(attrs = {'rows':1,'cols':5}),
          'email':Textarea(attrs = {'rows':1,'cols':5}),
          'phone_number':Textarea(attrs = {'rows':1,'cols':15}),
          'organisation':Textarea(attrs = {'rows':1,'cols':15}),
...
class Event_signup_form(forms.ModelForm):
    field_order = ['name','email','contact_number','linkedin_profile']
    class Meta:
        model = Community_Member
        exclude = ('highest_education','coder_profile',
